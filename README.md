...
ClipRadar는 방송 VOD에서 채팅 밀도, 클립 반응, 키워드 급증을 합쳐 하이라이트 후보를 보여주는 로컬 MVP입니다.
...
```text
구간 분당 채팅량 / 스트리머 평소 분당 채팅량
```

단순 채팅 수가 아니라 같은 방송자 기준으로 평소보다 얼마나 튀었는지 봅니다.
...
실시간 반응, 팬이 직접 자른 빈도, 잘린 클립의 소비량, 키워드 급증을 함께 봅니다.

### 레이더 신뢰도

```text
선정된 하이라이트들의 채팅 신호, 클립 신호, 키워드 신호 평균 일치도
```

레이더 신뢰도는 특정 하이라이트가 얼마나 높은 점수인지가 아니라, ClipRadar가 뽑은 후보들이 여러 신호에서 동시에 근거를 갖는지 보여줍니다.
